 ...
 *   c_outputFilename_numProc_tasks_cputime.dat,
 *   c_outputFilename_numProc_tasks_elapsedtime.dat,
 ...
 *
 * Usage:
 *   mpiexec -np <number of processes> bcastPerformanceMsgSize
 *	[message size] [output filename]
 *
 *
 * File: bcastPerformanceMsgSize.c	Author: S. Gross
 * Date: 20.05.2013
 *
 */
...
int main (int argc, char *argv[])
{
  ...
  char filename_cpu[MAX_FILENAME],
       ...
       numProcAsString[32];		/* # of processes as string	*/
  ...
  if (mytid == 0)
  {
    ...
    strncpy (filename_cpu + strlen (filename_cpu), "_",
	     MAX_FILENAME - strlen (filename_cpu));
    sprintf (numProcAsString, "%d", ntasks);
    strncpy (filename_cpu + strlen (filename_cpu), numProcAsString,
	     MAX_FILENAME - strlen (filename_cpu));
    strncpy (filename_cpu + strlen (filename_cpu), "_tasks",
	     MAX_FILENAME - strlen (filename_cpu));
  ...
  if (mytid == 0)
  {
    ...
    {
      fprintf (fp_cpu, "%d  %f\n", msgSize, sumCPUtime / CLOCKS_PER_SEC);
      fclose (fp_cpu);
    }
