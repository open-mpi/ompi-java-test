...
# compile the program
# "gcc" needs at least "-std=c99"
#
mpicc -std=c11 -o ${EXE_DIR}/bcastPerformanceNumProc \
  bcastPerformanceNumProc.c
...
for np in ${NUM_PROCS}; do
  for ms in ${MESSAGE_SIZES}; do
    ...
    mpiexec -np ${np} ${HOST_NAMES} ${MPI_OPTIONS} \
      bcastPerformanceNumProc ${ms} ${OUTPUT_FILENAME}
  done
done

# clean up
rm ${EXE_DIR}/bcastPerformanceNumProc
