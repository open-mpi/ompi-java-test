...
# (10, 100, ...) to the specified number of processes (2, 4, 8).
# This shell script varies the message size for a few number of
# processes.
#
...
# x-axis: message size, y-axis: time,
# one curve for each number of processes or each machine architecture
# in one graphic
#
...
#OUTPUT_FILENAME="Fedora_x86_64_tcp"
...
# compile the program
mpicc -o ${EXE_DIR}/bcastPerformanceMsgSize bcastPerformanceMsgSize.c
...
for np in ${NUM_PROCS}; do
  for ms in ${MESSAGE_SIZES}; do
    ...
    mpiexec -np ${np} ${HOST_NAMES} ${MPI_OPTIONS} \
      bcastPerformanceMsgSize ${ms} ${OUTPUT_FILENAME}
  done
done

# clean up
rm ${EXE_DIR}/bcastPerformanceMsgSize
