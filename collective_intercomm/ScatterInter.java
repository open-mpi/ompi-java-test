/* 
 *
 * This file is a port from "scatter_inter.c" from the "ompi-ibm-10.0"
 * regression test package. The formatting of the code is
 * mainly the same as in the original file.
 *
 *
 * File: ScatterInter.java		Author: S. Gross
 *
 */

import mpi.*;

public class ScatterInter
{
  ...
  public static void main (String args[]) throws MPIException
  {
    ...
    if (color != 0) {
      ...
      intercomm_scatter(intercomm, local_rank, -1);
      ...
      intercomm_scatter(intercomm, -1, local_rank);
      intercomm.disconnect();
    }
    comm.free();
    MPI.Finalize();
  ...
  private static void intercomm_scatter(Intercomm inter, int rank, int crank)
    throws MPIException
  {
    int out[],in[];
    int tasks;
	...
	inter.scatter(out,j,MPI.INT,in,j,MPI.INT,MPI.ROOT);
      } else if (rank == -1) {
	inter.scatter(out,j,MPI.INT,in,j,MPI.INT,root);
	for(int k = 0; k < j; k++) {
	  if(in[k] != k+crank*j) {  
	...
	inter.scatter(out,j,MPI.INT,in,j,MPI.INT,MPI.PROC_NULL);
      }
    }
  }
}
