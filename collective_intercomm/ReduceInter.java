/*
 *
 * This file is a port from "reduce_inter.c" from the "ompi-ibm-10.0"
 * regression test package. The formatting of the code is
 * mainly the same as in the original file.
 *
 *
 * File: ReduceInter.java		Author: S. Gross
 *
 */

import mpi.*;

public class ReduceInter
{
  ...
  public static void main (String args[]) throws MPIException
  {
    ...
    if (color != 0) {
      ...
      intercomm_reduce(intercomm, local_rank);
  ...
  private static void intercomm_reduce(Intercomm inter, int rank)
    throws MPIException
  {
	...
	out[i] = i;
      ...
      if (rank == 0) {
	inter.reduce(out, in, j, MPI.INT, MPI.SUM, MPI.ROOT);
	for (int k = 0; k < j; k++) {
	  if (in[k] != k * tasks) {
	...
	inter.reduce(out, in, j, MPI.INT, MPI.SUM, 0);
      } else {
	inter.reduce(out, in, j, MPI.INT, MPI.SUM, MPI.PROC_NULL);
      }
    }
  }
}
