/* 
 *
 * This file is a port from "ireduce_loc.c" from the "ompi-ibm-10.0"
 * regression test package. The formatting of the code is
 * mainly the same as in the original file.
 *
 *
 * File: IreduceLoc.java		Author: S. Gross
 *
 */
...
public class IreduceLoc
{
  ...
  private static void check_si(int rank, int size) throws MPIException
  {
	...
	request = MPI.COMM_WORLD.iReduce(out, in, count, MPI.SHORT_INT,
					 MPI.MINLOC, root);
	request.waitFor();
	request.free();
  ...
  private static void check_li(int rank, int size) throws MPIException
  {
	...
	request = MPI.COMM_WORLD.iReduce(out, in, count, MPI.LONG_INT,
					 MPI.MINLOC, root);
	request.waitFor();
	request.free();
